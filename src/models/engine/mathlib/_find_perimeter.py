...
def _get_perimeter(array, level, vertex_connect_high, returning_contours):
    """
    Versión pura en Python del cálculo de perímetro usando marching squares.
    """
    # Encuentra los contornos en el nivel dado
    contours = find_contours(array, level=level, fully_connected='high' if vertex_connect_high else 'low')
    perimeter = 0.0
    segments = []

    for contour in contours:
        # Suma la distancia entre puntos consecutivos para cada contorno
        dists = np.sqrt(np.sum(np.diff(contour, axis=0)**2, axis=1))
        perimeter += np.sum(dists)
        # Si se requieren los segmentos, agrégalos
        if returning_contours:
            for i in range(len(contour) - 1):
                segments.append((tuple(contour[i]), tuple(contour[i+1])))
